...
**진행:** Task 1~5 완료. Task 6 미착수. Task 7 은 Step 5 의 `OFFSET`(4.4 장) 칸만 남았다 — k6(4.1 장)는 2026-09-26 재측정 값으로 채웠다. **채택안은 C 안**(`brand_id, deleted_at, like_count DESC, id DESC`) — 측정 중에 추가된 제 3 안이다.
...
| 6 · `OFFSET` 관측 | 미착수 | |
| 7 · 설계 문서 반영 | Step 1~4 · 6 · 7 완료 (2026-09-22 17:15) — Step 5 는 4.1(k6) 을 2026-09-26 에 채웠고 4.4(`OFFSET`) 한 칸만 Task 6 대기 | `cc3f1fa8` |

**지금 빠져 있는 것은 실측 수치 하나다.** 판정의 본문은 설계 문서에 들어갔다 — 실측 격자는
3.6 장, 판정 근거는 3.7 장, k6 개선폭은 4.1 장이고, 가설 표(3.5 장)의 "실측" 칸 여섯 개도 채워져
있다. 남은 것은 Task 7 Step 5 의 한 칸 — 설계 4.4 장의 `OFFSET` 관측치(Task 6)다. 측정이 선행돼야
하므로 Task 7 은 Task 6 에 막혀 있는 것이지 착수되지 않은 것이 아니다.
...
- [ ] **Step 5: 4.1 · 4.3 · 4.4 · 4.5 장에 관측 수치를 넣는다** — **4.1 · 4.3 · 4.5 완료.**
