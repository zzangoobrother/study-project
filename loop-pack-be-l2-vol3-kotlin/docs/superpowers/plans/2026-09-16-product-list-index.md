...
**진행:** **Task 1~7 전부 완료 (2026-09-26).** 설계 문서 상태는 "측정 완료" 다. **채택안은 C 안**(`brand_id, deleted_at, like_count DESC, id DESC`) — 측정 중에 추가된 제 3 안이다.
...
| 4 · 채택안 코드 반영 | 완료 (2026-09-21 00:13) — Step 5(전체 스위트 750 건)는 2026-09-26 실행 | `3854e68f` |
| 5 · k6 end-to-end | 완료 (2026-09-24) — **도착률 30 TPS**(300 은 before 가 붕괴). 2026-09-26 재측정 — p95 1.43 배 · med 1.89 배 | `8cd7f276` · `bd4a957d` |
| 6 · `OFFSET` 관측 | 완료 (2026-09-26) — `OFFSET 90000` 에서 90,020 행 · 51.6 ms | `07c6afb6` |
| 7 · 설계 문서 반영 | 완료 — Step 1~4 · 6 · 7 은 2026-09-22, Step 5 의 4.1(k6) · 4.4(`OFFSET`) 는 2026-09-26 | `cc3f1fa8` · `a7196b5e` 외 |

**빠져 있는 실측 수치는 없다.** 실측 격자는 설계 3.6 장, 판정 근거는 3.7 장, k6 개선폭은 4.1 장,
`OFFSET` 관측은 4.4 장이고, 가설 표(3.5 장)의 "실측" 칸 여섯 개도 채워져 있다. 이 계획서 밖으로
넘긴 질문(`idx_products_brand_id` 제거 · 버퍼 풀 축소 · `latest`/`price_asc` · 커서 페이징 ·
마이그레이션 도구)은 설계 7 장에 있다.
...
**실측은 B 와 대등했다 — 더 빠른 것이 아니다.**

| | A 안 | B 안 | C 안 |
|---|---|---|---|
| count ① `Using index` | ✗ | ✓ | ✓ |
| count ① 중앙값 | 10.6 ms | 4.37 ms | 4.06 ms *(B 와 오차 범위)* |
| `idx_products_brand_id` 흡수 | ✓ | ✗ | **✓** |
| 도달 가능한 최종 인덱스 개수 | 2 | 3 | **2** |

**C 가 B 를 이긴 칸은 위 표의 아래 두 줄이지 count 중앙값이 아니다.** 5 회 원본에서 B 는
`4.02 ~ 5.15ms`, C 는 `3.95 ~ 4.63ms` 로 분포가 거의 겹친다. 같은 인덱스를 쓰는 B·C 의
경로 ② content 조차 0.416 / 0.337ms 로 갈리는 하네스다(설계 3.6 장). **C 의 우위는 실측이
아니라 구조에서 나온다** — 타이밍을 다시 재는 것으로는 이 판정이 뒤집히지 않는다.
...
> **2026-09-26 — Step 5 를 실행했다. 750 건 통과, 실패 · 에러 · 스킵 0** (`BUILD SUCCESSFUL in 1m 46s`,
> JUnit XML 집계). Task 5 전에 돌리겠다던 것을 Task 5 뒤에 돌렸다. 다만 `3854e68f` 이후
> `apps/` · `modules/` · `supports/` 에 커밋이 없으므로 **Task 4 가 남긴 코드 그대로를 검증한 것**이고,
> Task 5 결과에 영향을 주지 않는다 — k6 는 `@Index` 가 아니라 SQL 로 만든 인덱스 위에서 돌았다.
...
> ⚠️ **2026-09-26 — 위 표의 p95 · med · max · 200 건수는 워밍업 10 초가 섞인 값이다.** 당시
> `products.js` 의 요약이 전체 `http_req_duration` 을 읽었고, 200 건수 2,100 이 30 TPS × 70 초
> (워밍업 포함)와 정확히 맞는 것이 그 흔적이다. ~~after 의 max 486.5 ms 도 워밍업 첫 요청일 수
> 있다.~~ — **재측정에서 반증됐다**(아래 "재측정" 절): 워밍업을 포함한 after 의 max 도 27.84 ms 였다.
> 스크립트는 `bd4a957d` 에서 측정 구간만 읽도록 고쳤지만, 당시 결과 JSON 에는 측정 구간 분포가
> 기록되지 않아 사후 보정이 불가능하다. **이 수치를 설계 4.1 장에 옮기지 않는다 — 30 TPS 재측정
> 값으로 대체한다.** 방향(after 가 빠르다)에 대한 판단은 유지하되 크기는 확정된 근거가 아니다.
...
**② 측정 순서가 `after` → `before` 가 아니라 다음과 같았다.**

```
after(300) → before(300, 붕괴) → before(10 ~ 100 도착률 곡선) → before(30) → after(30)
```

Step 2 를 300 TPS 로 먼저 돌린 뒤 `before` 에서 붕괴를 발견했고, 위 ① 의 곡선을 그리느라
`before` 를 여러 도착률에서 쟀다. 도착률을 30 으로 정한 다음에는 `after` 도 30 TPS 에서 다시
재야 했다.

> **2026-09-26 정정.** 이 절은 원래 `after`(300) → `before`(30) → `after`(30) 로 적혀 있었다.
> 같은 절의 도착률 곡선이 `before` 를 300 · 10~100 에서 잰 기록이므로 그 서술은 중간을 빠뜨린
> 것이다. 또 **마지막 `after`(30) 직전, 인덱스를 다시 만든 뒤 버리는 실행 2 회를 돌렸는지는
> 기록이 없다.** Task 3 실행 결과 ① 이 "`CREATE INDEX` 직후 첫 측정이 가장 느리다" 를 찾아냈으므로,
> 30 TPS 재측정 때는 인덱스 재생성 → 버리는 실행 2 회 → 측정 순서를 지키고 그 사실을 적는다. 앱은 한 번도 재기동하지 않았고 인덱스만 SQL 로
...
**이 절의 p95 수치는 설계 문서 4.1 장에 들어가지 않았다.** 설계 4.1 장에 들어간 것은 아래
재측정 값이다.

### 재측정 (2026-09-26, 스크립트 수정 `bd4a957d` 이후)

위 측정의 두 결함 — 요약에 워밍업이 섞인 것, 인덱스 재생성 뒤 버리는 실행 2 회의 기록이 없는 것 —
을 걷어내고 30 TPS 에서 다시 쟀다.

**절차.** `down -v` 로 새로 띄움(앱 기동 직후 `products` 137 행 — 스키마 재생성 확인) → 재시드 →
`verify-seed.sql` 기대값 전부 일치 → C 안 인덱스 생성(`Collation` `D` 확인) → **after**: 버리는
실행 2 회 → 측정 → 인덱스 `DROP` → **before**: 버리는 실행 2 회 → 측정 → 인덱스 복구. 앱은 한 번도
재기동하지 않았다. 모든 실행이 `dropped_iterations = 0`, 200 건수 1,801(= 30 TPS × 60 초, 측정 구간만).

| | p95 | med | max | 200 | dropped |
|---|---:|---:|---:|---:|---:|
| before (인덱스 없음) | 22.18 ms | 20.40 ms | 42.30 ms | 1,801 | 0 |
| after (C 안) | **15.46 ms** | **10.81 ms** | 27.84 ms | 1,801 | 0 |
| 개선 | **1.43 배 (−30.3%)** | **1.89 배 (−47.0%)** | | | |

버리는 실행의 p95 는 after 14.0 · 15.2 ms, before 22.1 · 22.2 ms 로 본 측정과 같은 자리였다.

**예상과 달랐던 것 — 첫 측정을 흐린 것은 워밍업이 아니었을 가능성이 크다.** 같은 JSON 에서
워밍업을 포함한 전체 `http_req_duration` 도 읽어 봤다.

```
                 측정 구간만                      워밍업 포함
before   p95 22.18  med 20.40  max 42.30    p95 22.18  med 20.42  max 51.84
after    p95 15.46  med 10.81  max 27.84    p95 15.49  med 10.92  max 27.84
```

워밍업을 섞어도 p95 · med 는 0.1 ms 안팎만 움직였다. 그러니 첫 측정 after 의 max 486.5 ms 는
워밍업으로 설명되지 않는다. 이번과 달랐던 점은 **인덱스 재생성 뒤 버리는 실행 2 회**가 있었느냐이고,
Task 3 실행 결과 ① 의 "`CREATE INDEX` 직후 첫 측정이 가장 느리다" 가 더 그럴듯한 원인이다 —
**추정이지 확인은 아니다.** 스크립트 수정은 그래도 맞다. 요약이 무엇을 세는지가 명시되지 않으면
이번처럼 "무엇이 흐렸는가" 를 가려낼 수 없다.

첫 측정과 비교하면 p95 는 거의 같고(15.7 → 15.46 ms) after 의 med 가 달라졌다(11.9 → 10.81 ms).
그래서 med 개선폭이 1.70 배에서 1.89 배로 커졌다.

**설계 5.6 장의 게이트는 절반만 확인됐다.** k6 는 before 와 C 만 쟀으므로 "인덱스 효과의 방향이
API 까지 살아남는다" 는 확인됐지만, 그 게이트가 적은 "k6 에서 A 나 B 가 C 보다 빠른가" 는 재지
않았다. 위 첫 측정 절의 "게이트는 통과했다" 도 이 범위로 읽는다 (설계 5.6 장 인용 블록).
...
### 실행 결과 (2026-09-26, commit `07c6afb6`)

Step 2 의 기대가 그대로 나왔다 — `OFFSET 90000` 의 `actual rows` 는 정확히 **90,020** 이다.
측정 컨테이너는 Task 5 재측정 직후 그대로라 C 안 인덱스가 걸려 있었고, 두 쿼리 모두 filesort 없이
인덱스를 탔다.

| | 경로 | `key` | `actual rows` | `actual time` (5 회 중앙값) |
|---|---|---|---:|---:|
| `OFFSET 0` | ① | `idx_products_brand_del_like` | 20 | 0.283 ms |
| `OFFSET 90000` | ② | `idx_products_del_like` | 90,020 | 51.6 ms |

**절차가 한 군데 바뀌었다 — 5 회 반복을 더했다(`limits-5runs.txt`).** Step 2 는 1 회 측정이었는데,
캡처(56 ms)가 **개선 전 경로 ② `OFFSET 0`(전체 스캔 + filesort, 35.1 ms)보다 느렸다.** 인덱스를
타고도 인덱스 없는 첫 페이지보다 느린지는 1 회 값으로 판단할 수 없어 Task 3 의 5 회 규약을 따랐고,
중앙값 51.6 ms(49.0 ~ 61.3)로 방향은 유지됐다. 다만 비교 대상 35.1 ms 가 1 회 측정이라 설계 4.4 장에는
"방향만 참고" 로 적었다. 커밋 대상에 `limits-5runs.txt` 가 더해진 것도 이 때문이다.

**계획의 `OFFSET 0` 은 경로 ① 이라 `OFFSET 90000`(경로 ②)과 경로가 다르다.** 설계 4.4 장에는
같은 경로 ② 의 `OFFSET 0`(3.6 장 격자, C 안 0.337 ms)을 함께 넣어 비교를 맞췄다 — 읽은 행 4,501 배,
시간 약 150 배다.
...
- [x] **Step 5: 4.1 · 4.3 · 4.4 · 4.5 장에 관측 수치를 넣는다** — **네 장 모두 완료.**
  4.4(`OFFSET` 관측치)는 2026-09-26 Task 6 결과로 채웠고, 함께 상태 줄을 "측정 완료 (2026-09-26)" 로,
  7 장 커서 페이징 질문에 출발점 수치를 반영했다. 아래는 채우기 전의 기록이다.
  4.1(k6 개선폭)은 2026-09-26 Task 5 재측정 값으로 채웠다(함께 5.5 장에 도착률 30 TPS 의 근거,
  5.6 장에 게이트가 확인한 범위, 상태 줄을 반영). 4.4(`OFFSET` 관측치)는 Task 6 이 끝나야 넣을 수 있다. 설계 문서의
...
> **2026-09-26 — 상태 줄이 Step 7 의 원래 형태로 돌아왔다.** Task 5 재측정과 Task 6 이 끝나
> 남은 측정이 없어졌으므로 `상태: **측정 완료 (2026-09-26)**` 로 바꿨다. 날짜는 마지막으로 측정한
> 날이다 — 인덱스 판정일(2026-09-20)은 상태 줄 본문에 따로 남겼다.
...
- [x] 전체 테스트 750 건 통과 (기존 748 + 인덱스 테스트 2) — 2026-09-26 Task 4 Step 5 실행
