...
public class CommandHandler {
    ...
    private Boolean register(String[] params) {
        if (params.length > 1) {
            if (restApiService.register(params[0], params[1])) {
                terminalService.printSystemMessage("Register.");
            } else {
                terminalService.printSystemMessage("Register fail");
            }
        }
    ...
    private Boolean unregister(String[] params) {
        ...
        if (restApiService.unregister()) {
            terminalService.printSystemMessage("unregister");
        } else {
            terminalService.printSystemMessage("unregister fail");
        }
        ...
        if (params.length > 1) {
            if (restApiService.login(params[0], params[1])) {
                if (webSocketService.createSession(restApiService.getSessionId())) {
                    terminalService.printSystemMessage("login");
                }
            } else {
                terminalService.printSystemMessage("login fail");
            }
        }
    ...
    private Boolean logout(String[] params) {
        ...
        if (restApiService.logout()) {
            terminalService.printSystemMessage("logout");
        } else {
            terminalService.printSystemMessage("logout fail");
        }
    ...
    private Boolean clear(String[] params) {
        ...
        terminalService.printSystemMessage("clear");
        return true;
    }

    private Boolean exit(String[] params) {
        webSocketService.closeSession();
        terminalService.printSystemMessage("exit");
        return false;
    }
}
