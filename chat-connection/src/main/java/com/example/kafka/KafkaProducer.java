...
public class KafkaProducer {
    ...
    public void sendMessageUsingPartitionKey(ChannelId channelId, UserId userId, RecordInterface recordInterface, Runnable errorCallback) {
        ...
        jsonUtil.toJson(recordInterface).ifPresent(record -> kafkaTemplate.send(pushTopic, record).whenComplete(logResult(pushTopic, record, partitionKey, errorCallback)));
    }

    public void sendRequest(RecordInterface recordInterface, Runnable errorCallback) {
        jsonUtil.toJson(recordInterface).ifPresent(record -> kafkaTemplate.send(pushTopic, record).whenComplete(logResult(pushTopic, record, null, errorCallback)));
    }
    ...
    private BiConsumer<SendResult<String, String>, Throwable> logResult(String topic, String record, String partitionKey, Runnable errorCallback) {
        return (sendResult, throwable) -> {
            if (throwable == null) {
                ...
                log.info("Record produced failed : {} with key : {} to topic : {}, cause : {}", record, partitionKey, pushTopic, throwable.getMessage());
